# setup.ps1
# Run this from inside your dashboard/ folder:
#   cd iisc\frontend\dashboard
#   powershell -ExecutionPolicy Bypass -File setup.ps1

Write-Host "Setting up AccessQR dashboard..." -ForegroundColor Cyan

# ── Delete CRA default files ──────────────────────────────────────
$toDelete = @("src\App.test.js","src\logo.svg","src\reportWebVitals.js","src\setupTests.js")
foreach ($f in $toDelete) { if (Test-Path $f) { Remove-Item $f; Write-Host "Deleted $f" } }

# ── Create folders ────────────────────────────────────────────────
$dirs = @("src\context","src\hooks","src\utils","src\components","src\screens")
...
export default function App() {
  ...
  return (
    <AppContext.Provider value={ctx}>
      ...
      {deleteTarget && (
        <DeleteModal name={deleteTarget.name} onCancel={() => setDeleteTarget(null)} onConfirm={handleDeleteConfirm} />
      )}
    </AppContext.Provider>
  );
}
...
# ════════════════════════════════════════════════════════════════════
# src\context\AppContext.js   <-- EXACT filename, capital C
# ════════════════════════════════════════════════════════════════════
Write-File "src\context\AppContext.js" @'
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\hooks\useProducts.js" @'
...
const SEED = [
  ...
  { id:'PRD002', name:'Paracetamol 500mg', cat:'Medicine', price:'22.00', expiry:'2026-06-15', ingredients:'Paracetamol 500mg, Microcrystalline cellulose, Starch', warnings:'Do not exceed 4 doses in 24 hours. Keep away from children.', date:'Mar 17, 2026' },
  ...
  { id:'PRD004', name:'Dove Body Lotion', cat:'Personal Care', price:'185.00', expiry:'2027-01-01', ingredients:'Water, Glycerin, Mineral Oil, Dimethicone', warnings:'For external use only. Avoid contact with eyes.', date:'Mar 14, 2026' },
];
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\utils\helpers.js" @'
export const CATEGORIES = ['Groceries','Instant Food','Medicine','Personal Care','Household','Dairy','Beverages'];

export const CAT_BADGE = {
  'Groceries':'blue','Instant Food':'green','Medicine':'amber',
  'Personal Care':'neutral','Household':'neutral','Dairy':'blue','Beverages':'green',
};
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\Sidebar.css" @'
.sidebar { width:var(--sidebar); flex-shrink:0; background:var(--surface); border-right:1px solid var(--border); display:flex; flex-direction:column; position:sticky; top:0; height:100vh; overflow:hidden; }
...
.brand-tag { font-size:10px; color:var(--text3); letter-spacing:0.3px; margin-top:1px; }
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\Sidebar.js" @'
import React from 'react';
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\Topbar.js" @'
import React from 'react';
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\Toast.js" @'
...
# ════════════════════════════════════════════════════════════════════
# src\components\DeleteModal.js   <-- EXACT name capital D capital M
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\DeleteModal.js" @'
    ...
    <div onClick={(e) => { if (e.target === e.currentTarget) onCancel(); }} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.35)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}}>
      <div className="fade-up" style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-lg)',padding:24,width:340}}>
        ...
        <p style={{fontSize:13,color:'var(--text2)',marginBottom:20,lineHeight:1.6}}>
          This will permanently remove <strong style={{color:'var(--text)'}}>{name}</strong> and its QR code. This action cannot be undone.
        </p>
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\UI.js" @'
import React, { useState } from 'react';
...
const BADGE_CSS = {
  ...
  neutral: { background:'var(--surface2)', color:'var(--text2)' },
};
...
const BTN_HOVER = { primary:{background:'#2d2c28'}, secondary:{background:'var(--surface2)',borderColor:'var(--border2)'}, ghost:{background:'var(--surface2)',color:'var(--text)'}, danger:{background:'#fad9d9'} };
const BTN_PAD   = { lg:'11px 20px', md:'8px 14px', sm:'5px 10px' };
const BTN_SIZE  = { lg:14, md:13, sm:12 };
...
export function Button({ children, variant='secondary', size='md', onClick, disabled, style, icon }) {
  ...
  const hover = hov ? (BTN_HOVER[variant] || {}) : {};
  return (
    <button onClick={onClick} disabled={disabled} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{display:'inline-flex',alignItems:'center',gap:7,padding:BTN_PAD[size],borderRadius:'var(--r)',fontSize:BTN_SIZE[size],fontWeight:500,border:`1px solid ${base.borderColor}`,cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.5:1,transition:'all 0.15s ease',...base,...hover,...style}}>
...
const INPUT_BASE = { width:'100%', padding:'9px 12px', border:'1px solid var(--border)', borderRadius:'var(--r)', fontSize:13, color:'var(--text)', background:'var(--surface)', outline:'none', transition:'border-color 0.15s, box-shadow 0.15s', WebkitAppearance:'none' };

export function Input({ value, onChange, placeholder, type='text', style, ...rest }) {
  const [foc, setFoc] = useState(false);
  return <input type={type} value={value} onChange={onChange} placeholder={placeholder} onFocus={() => setFoc(true)} onBlur={() => setFoc(false)} style={{...INPUT_BASE,borderColor:foc?'var(--text2)':'var(--border)',boxShadow:foc?'0 0 0 3px rgba(26,25,22,0.07)':'none',...style}} {...rest}/>;
}

export function Select({ value, onChange, children, style }) {
  const [foc, setFoc] = useState(false);
  return <select value={value} onChange={onChange} onFocus={() => setFoc(true)} onBlur={() => setFoc(false)} style={{...INPUT_BASE,cursor:'pointer',borderColor:foc?'var(--text2)':'var(--border)',boxShadow:foc?'0 0 0 3px rgba(26,25,22,0.07)':'none',...style}}>{children}</select>;
}

export function Textarea({ value, onChange, placeholder, minHeight=80 }) {
  const [foc, setFoc] = useState(false);
  return <textarea value={value} onChange={onChange} placeholder={placeholder} onFocus={() => setFoc(true)} onBlur={() => setFoc(false)} style={{...INPUT_BASE,resize:'vertical',minHeight,lineHeight:1.5,borderColor:foc?'var(--text2)':'var(--border)',boxShadow:foc?'0 0 0 3px rgba(26,25,22,0.07)':'none'}}/>;
}
...
export function IconBtn({ d, onClick, title, danger }) {
  ...
  return (
    <button onClick={onClick} title={title} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:28,height:28,borderRadius:'var(--r)',background:hov?(danger?'var(--red-bg)':'var(--surface2)'):'transparent',border:'none',cursor:'pointer',transition:'all 0.15s',color:danger?'var(--red)':'var(--text2)'}}>
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\components\ProductForm.js" @'
import React from 'react';
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\screens\Dashboard.js" @'
import React from 'react';
import { useApp } from '../context/AppContext';
        ...
        <div style={{width:230,flexShrink:0,display:'flex',flexDirection:'column',gap:12}}>
          <Card padding="16px 18px">
            ...
            <div style={{marginTop:12,display:'flex',flexDirection:'column',gap:8}}>
              ...
              <Button variant="secondary" onClick={() => navigate('products')} style={{justifyContent:'center',width:'100%'}} icon="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01">View all products</Button>
            </div>
          </Card>
          <Card padding="16px 18px">
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\screens\AddProduct.js" @'
import React, { useState } from 'react';
import { useApp } from '../context/AppContext';
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\screens\EditProduct.js" @'
import React, { useState } from 'react';
import { useApp } from '../context/AppContext';
import { PageHeader } from '../components/UI';
import ProductForm from '../components/ProductForm';
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\screens\ProductList.js" @'
import React, { useState } from 'react';
import { useApp } from '../context/AppContext';
...
export default function ProductList() {
  ...
  const filtered = products.filter(p => {
    ...
    const matchQ = !q || p.name.toLowerCase().includes(q) || p.id.toLowerCase().includes(q) || p.cat.toLowerCase().includes(q);
  ...
  return (
    <div className="fade-up">
      <PageHeader title="Products" subtitle={products.length + ' product' + (products.length !== 1 ? 's' : '') + ' total'}
        action={<Button variant="primary" onClick={() => navigate('add')} icon="M12 5v14M5 12h14">Add product</Button>}/>
        ...
        <table style={{width:'100%',borderCollapse:'collapse',background:'var(--surface)',tableLayout:'fixed'}}>
          ...
          <thead>
            <tr style={{background:'var(--surface2)',borderBottom:'1px solid var(--border)'}}>
              {['Product','Category','Price','Last updated','Actions'].map(h => (
                <th key={h} style={{padding:'10px 16px',fontSize:11,fontWeight:600,letterSpacing:'0.5px',textTransform:'uppercase',color:'var(--text3)',textAlign:'left'}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={5}><div style={{textAlign:'center',padding:'48px 20px'}}>
                <div style={{width:40,height:40,borderRadius:10,background:'var(--surface2)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 12px'}}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="var(--text3)" strokeWidth="1.5" strokeLinecap="round" style={{width:18,height:18}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                </div>
                <div style={{fontSize:13,fontWeight:500,color:'var(--text2)',marginBottom:4}}>{search||catFilter?'No products match your search':'No products yet'}</div>
                <div style={{fontSize:12,color:'var(--text3)'}}>{search||catFilter?'Try a different search term':'Add your first product to get started'}</div>
              </div></td></tr>
    ...
    <tr onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{borderBottom:isLast?'none':'1px solid var(--border)',background:hov?'var(--surface2)':'var(--surface)',transition:'background 0.1s'}}>
      ...
      <td style={{padding:'12px 16px'}}>
        <div style={{display:'flex',alignItems:'center',gap:2}}>
          <IconBtn d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6z" onClick={onView} title="View output"/>
...
# ════════════════════════════════════════════════════════════════════
Write-File "src\screens\OutputScreen.js" @'
...
function QRDisplay({ productId }) {
  ...
  for (let r = 0; r < 7; r++) for (let c = 0; c < 7; c++) {
    const filled = fixed.has(r+','+c) || rand() > 0.45;
    cells.push(<div key={r+'-'+c} style={{borderRadius:1,background:filled?'#F7F6F3':'transparent'}}/>);
  }
...
function AudioPlayer({ description }) {
  ...
  const handlePlay = () => {
    if (playing) {
      clearInterval(iRef.current); window.speechSynthesis?.cancel(); setPlaying(false);
    } else {
      setPlaying(true); setProgress(0);
      if ('speechSynthesis' in window) {
        window.speechSynthesis.cancel();
        const u = new SpeechSynthesisUtterance(description); u.rate=0.9;
        u.onend = () => { setPlaying(false); setProgress(0); };
        window.speechSynthesis.speak(u);
      }
      let p = 0;
      iRef.current = setInterval(() => { p++; setProgress(p); if (p >= BARS) { clearInterval(iRef.current); setPlaying(false); setProgress(0); } }, 150);
    }
  };
...
export default function OutputScreen({ product }) {
  ...
  if (!product) return <div className="fade-up" style={{textAlign:'center',paddingTop:60,color:'var(--text3)'}}>No product to display. <button onClick={() => navigate('products')} style={{color:'var(--blue)',border:'none',background:'none',cursor:'pointer',fontSize:14}}>Go to products</button></div>;
      ...
      <div style={{display:'grid',gridTemplateColumns:'200px 1fr',gap:20,alignItems:'start'}}>
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-lg)',padding:'20px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
          ...
          <div style={{textAlign:'center'}}>
            <div style={{fontSize:11,fontFamily:'var(--mono)',color:'var(--text3)',marginBottom:2}}>{product.id}</div>
            <div style={{fontSize:11,color:'var(--text3)'}}>Scan with AccessQR app</div>
          </div>
          <Button variant="primary" onClick={() => alert('QR for ' + product.name + ' (ID: ' + product.id + ') would download here.')} style={{width:'100%',justifyContent:'center'}} icon="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4 M7 10l5 5 5-5 M12 15V3">Download QR</Button>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <Card padding="0">
            <div style={{padding:'13px 18px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              ...
              <Badge label="Auto-generated" variant="green"/>
            </div>
          ...
          <Card>
            ...
            <table style={{width:'100%',fontSize:13,borderCollapse:'collapse',marginTop:12}}>
              <tbody>
                {[{label:'Name',value:product.name},{label:'Category',value:<Badge label={product.cat} variant={CAT_BADGE[product.cat]}/>},{label:'Price',value:formatPrice(product.price)},{label:'Expiry',value:formatDate(product.expiry)},{label:'Ingredients',value:product.ingredients||'--'},{label:'Warnings',value:product.warnings||'--'}].map(({label,value}) => (
          ...
          <div style={{display:'flex',gap:8}}>
            <Button variant="secondary" onClick={() => navigate('products')} icon="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01">View all products</Button>
